...
export const registerUserController = AsyncHandler(async (req,res) => {
    const {name , email , password} = req.body;
     ...
     // create new token
     const token = jwt.sign({_id : newUser._id} , process.env.JWT_SECRET as string , {expiresIn : "7d"});
...
export const signInController = AsyncHandler(async(req , res) => {
     ...
     // create new token
     const token = jwt.sign({_id : existingUser._id} , process.env.JWT_SECRET as string , {expiresIn : "7d"});
